...
# Binaries and rules for building binaries
BINARIES := \
	promcli 
...
$(BINARY)-no-deps:
	go build -gcflags="$(GO_BUILD_GC_FLAGS)" -ldflags "$(GO_BUILD_LD_FLAGS)" -o "$(BUILD)/$(BINARY)" ./src/cmd/$(BINARY)/main/.

$(BINARY)-darwin-amd64: setup
	$(DARWIN_AMD64_ENV) make $(BINARY)-no-deps

$(BINARY)-darwin-arm64: setup
	$(DARWIN_ARM64_ENV) make $(BINARY)-no-deps
