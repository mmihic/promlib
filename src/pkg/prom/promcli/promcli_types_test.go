...
func TestTime_UnmarshalText_Absolute(t *testing.T) {
	...
	err = json.Unmarshal([]byte(`{"my-time": "2023-05-06"}`), &value)
	require.NoError(t, err)
	assert.Equal(t, value.MyTime, Time(timex.MustParseTime(time.RFC3339, "2023-05-06T00:00:00Z")))

	err = json.Unmarshal([]byte(`{"my-time": "2023-05"}`), &value)
	require.NoError(t, err)
	assert.Equal(t, value.MyTime, Time(timex.MustParseTime(time.RFC3339, "2023-05-01T00:00:00Z")))
}
...
func TestTime_UnmarshalText_Relative(t *testing.T) {
	...
	halfHourFromNow := time.Now().Add(time.Minute * 30)
	assert.True(t, value.MyTime.AsTime().Equal(halfHourFromNow) || value.MyTime.AsTime().Before(halfHourFromNow))
